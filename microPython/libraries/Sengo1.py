...
class VisionState:
    ...
    def sortd(self):
        for i in range(self.detect-1):
            for j in range(self.detect-i-1):
                if self.compare(self.result[j],self.result[j+1]) > 0 :
    ...
    def compare(self,r1:result,r2:result):  
        res = 1
...
class SentryI2CMethod:
    ...
    def Read(self, vision_type, vision_state):
        ...
        if sengo1_vision_e.kVisionQrCode == vision_type:
...
class SentryBase:
    ...
    def GetVisionState(self, vision_type):
        if vision_type >= sengo1_vision_e.kVisionMaxType:
            return 0
    ...
    def GetQrCodeString(self):
        vision_state = self.__vision_states[sengo1_vision_e.kVisionQrCode-1]
...
class Sengo1(SentryBase):
    ...
    def __init__(self, address=0x60, log_level=LOG_ERROR):
        super().__init__(self.sengo1_DEVICE_ID,address,log_level)